...
const findBemas = (token, pnyx) => {
	return fetch(`/api/bema/persona/${pnyx}/list`, {
		method: 'GET',
		headers: {
...
const PersonaPnyx = props => {
	...
	const [pnyx, setPnyx] = useState(false)
	...
	useEffect(() => {
		if (persona && personaName) {
			setNamed(
				personaName === 'me' ? persona.persona.personaName : personaName
			)
		}
	}, [persona, personaName])
	...
	useEffect(() => {
		if (named) {
			setPnyx(`persona-${named}`)
		}
	}, [named])

	useEffect(() => {
		if (allowed && refresh && pnyx) {
			console.log('refreshing pnyx')
			openLenny()
			persona.auth
				.getIdToken(true)
				.then(idToken => {
					return findBemas(idToken, pnyx)
				})
	...
	}, [allowed, refresh, props.timedout, pnyx])
