...
const findMessageBemas = (token, authentic) => {
	return fetch(`/api/persona/item/${authentic.persona.UID}/messages/list`, {
		method: 'GET',
		headers: {
...
const Messages = props => {
	...
	useEffect(() => {
		if (allowed && refresh && persona) {
			console.log('refreshing pnyx')
			openLenny()
			persona.auth
				.getIdToken(true)
				.then(idToken => {
					return findMessageBemas(idToken, persona)
				})
	...
	}, [allowed, refresh, persona, props.timedout])
